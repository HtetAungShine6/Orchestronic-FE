...
export function AzureVMSizeCombobox({
  ...
  const {
    ...
    queryKey: ["azure-vm-sizes", search],
    queryFn: () => fetchVmSizes(search, 1, 20, usePolicyFilter),
  })
      ...
      <PopoverTrigger asChild>
        ...
        >
          <span className="truncate">
            {selectedValue?.name || "Select VM Size..."}
          </span>
          <ChevronsUpDown className="opacity-50 ml-2 h-4 w-4 shrink-0" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-[400px] p-0 z-50" portal>
        <Command>
          <CommandInput
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-1 text-xs text-muted-foreground">
                      <div>vCPUs: {vm.numberOfCores}</div>
                      <div>RAM: {Math.round(vm.memoryInMB / 1024)} GB</div>
                      <div>
                        OS Disk: {Math.round(vm.osDiskSizeInMB / 1024)} GB
                      </div>
                      <div>Max Disks: {vm.maxDataDiskCount}</div>
                    </div>
                  </div>
                  <Check
                    className={`h-4 w-4 shrink-0 ${selectedValue?.id === vm.id ? "opacity-100" : "opacity-0"}`}
                  />
                </CommandItem>
              ))}
            </CommandGroup>
